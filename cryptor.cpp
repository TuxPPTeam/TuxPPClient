...
/**
 * Drops given number of precomputed bytes.
 * @param n number of bytes to drop
 * @return number of available bytes dropped
 */
size_t Cryptor::drop(size_t n) {
    size_t dropped = qMin(n, availableBytes);
...
//        }
    ...
    qDebug() << "Encryption speed:" << (static_cast<double>(processed) / timer.elapsed()) / 1000 << "MB/s";
...
//        ctr_drbg_free(&ctr_drbg);
    ...
    if( ctr_drbg_random( &ctr_drbg, data, size ) != 0 )
    {
...
//    ctr_drbg_free(&ctr_drbg);
    QByteArray toReturn = QByteArray((char*) data, size);
    delete[] data;
    return toReturn;
}
...
QByteArray Cryptor::encryptRSA(QByteArray data, QString pubKeyPath) {
    ...
    pubkey = pkfile.readAll();
    pkfile.close();

    return encryptRSA(data, pubkey);
}
